 ...
 */
open class GoHTMXWebViewClient : WebViewClient() {

    companion object {
        const val SCHEME = "gohtmx"
        const val HOST = "app"
    }
    ...
    override fun shouldInterceptRequest(
        ...
        // Only intercept gohtmx:// scheme
        if (url.scheme != SCHEME) {
            return super.shouldInterceptRequest(view, request)
        }

        // Convert gohtmx://app/path?query -> /path?query
        var path = url.path ?: "/"
        if (path.isEmpty()) {
            path = "/"
        }
        ...
        val response = GoHTMXBridge.handleRequest(
            method = method,
            url = path,
            headers = headers,
