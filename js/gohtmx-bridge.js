/**
 * GoHTMX Bridge - Virtual HTTP and WebSocket for HTMX 4
 *
...
(function() {
    ...
    const isIOS = !!(window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.gohtmx);
    const isAndroid = typeof window.GoHTMX !== 'undefined';
    ...
    const NativeBridge = {
        ...
        async httpRequest(method, url, headers, body) {
            if (isIOS) {
                return new Promise((resolve, reject) => {
                    ...
                    window.webkit.messageHandlers.gohtmx.postMessage({
                        type: 'http',
                        requestId,
                        method,
                        url,
                ...
                const response = window.GoHTMX.handleRequest(
                    method,
                    url,
        ...
        async wsConnect(url) {
            if (isIOS) {
                ...
                return window.GoHTMX.wsConnect(url);
            }
            throw new Error('Native bridge not available');
        },
        ...
        wsSend(sessionId, data) {
            if (isIOS) {
                ...
                window.GoHTMX.wsSend(sessionId, data);
            }
        },
        ...
        wsClose(sessionId, code, reason) {
            if (isIOS) {
                ...
                window.GoHTMX.wsClose(sessionId, code || 1000, reason || '');
            }
        }
    };
    ...
    window._gohtmx_http_response = function(requestId, status, headers, body) {
    ...
    window._gohtmx_http_error = function(requestId, error) {
        const pending = pendingHttpRequests.get(requestId);
        if (pending) {
            pendingHttpRequests.delete(requestId);
    ...
    window._gohtmx_ws_connected = function(requestId, sessionId) {
    ...
    window._gohtmx_ws_connect_error = function(requestId, error) {
        const pending = pendingWsConnects.get(requestId);
        if (pending) {
            pendingWsConnects.delete(requestId);
            pending.reject(new Error(error));
    ...
    window._gohtmx_ws_message = function(sessionId, data) {
    ...
    window._gohtmx_ws_close = function(sessionId, code, reason) {
        const ws = VirtualWebSocket._sessions.get(sessionId);
        if (ws) {
    ...
    window._gohtmx_ws_error = function(sessionId, error) {
        const ws = VirtualWebSocket._sessions.get(sessionId);
        if (ws) {
    ...
    if (typeof htmx !== 'undefined') {
        htmx.defineExtension('gohtmx-bridge', {
            init: function(api) {
                console.log('[gohtmx] Bridge extension initialized, native:', isNative);
            },
            ...
            onEvent: function(name, evt) {
                ...
                if (name === 'htmx:beforeRequest') {
                    ...
                    // Convert to gohtmx:// scheme if relative
                    if (url && !url.startsWith('http') && !url.startsWith('gohtmx://')) {
                        url = 'gohtmx://app' + (url.startsWith('/') ? '' : '/') + url;
                    }
                    ...
                    NativeBridge.httpRequest(method, url, headers, body)
                        ...
                        .catch(error => {
                            console.error('[gohtmx] Request error:', error);
    ...
    function normalizeWebSocketUrl(url) {
        ...
        // Relative URL - use gohtmx:// scheme for native
        if (isNative) {
            return 'gohtmx://ws' + (url.startsWith('/') ? '' : '/') + url;
        }
    ...
    console.log('[gohtmx] Bridge loaded, platform:', isNative ? (isIOS ? 'iOS' : 'Android') : 'web');
